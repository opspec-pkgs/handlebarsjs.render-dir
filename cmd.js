...
/**
 * processes a single handlebars helper file
 * @param {*} absPath
 */
const processHelperFile = (absPath) => {
    const helper = require(absPath);
    const helperName = absPath.slice('/helpers/'.length, absPath.length - (path.parse(absPath).ext).length);
...
walk('/helpers', (absPath, stat) => {
    if(!stat.isDirectory()) {
    processHelperFile(absPath);
}
});
